...
| `5631330` | **Known service cost blocker** | A `simple_qna` generation request for 15 produced a reported result/file count of 16; the separate seed case returned 15. A fix is not established for the reported Q&A generation path. A merged fix or a successful seed count does not establish applicability to that path; this is not solely an unconfirmed-rollout question. Generation counts are distinct from dataset-run caps. |
